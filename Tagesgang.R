...
staFiles <- list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/sta/", full.names = T, pattern = ".csv$")
staNames <- gsub(".csv", "", list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/sta/", full.names = F, pattern = ".csv$"))
...
for(i in 1:length(staFiles)){
    ...
    write.csv2(result, file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/", staNames[i], ".csv"), row.names = F)
}


staFiles <- list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/", full.names = T, pattern = ".csv$")
staNames <- gsub(".csv", "", list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/", full.names = F, pattern = ".csv$"))
...
for(i in 1:length(staFiles)){
    ...
    ggsave(filename=paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/plot120/STA_", staNames[i], "_120.png"),
           plot=e, width = 19, height = 9)
    
    e <- ggplot(resultFrame, aes(x=TIMESLOT60, fill=SYS)) + ggtitle(paste("STA ", staNames[i])) + geom_bar() + 
        theme_minimal() + scale_fill_brewer(palette="Set1") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        scale_x_discrete(drop=FALSE) + facet_grid(DIRECTION~.)
    ggsave(filename=paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/plot60/STA_", staNames[i], "_60.png"),
           plot=e, width = 19, height = 9)
    
    e <- ggplot(resultFrame, aes(x=TIMESLOT30, fill=SYS)) + ggtitle(paste("STA ", staNames[i])) + geom_bar() + 
        theme_minimal() + scale_fill_brewer(palette="Set1") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        scale_x_discrete(drop=FALSE) + facet_grid(DIRECTION~.)
    ggsave(filename=paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/plot30/STA_", staNames[i], "_30.png"),
           plot=e, width = 19, height = 9)
    ...
    write.csv2(table(resultFrame$TIMESLOT60[resultFrame$DIRECTION == "Ri"], 
               ...
               file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables/STA_", staNames[i], "_Ri_",
                             fBTS, "-", lBTS,".csv"), 
               row.names = T)
    
    write.csv2(table(resultFrame$TIMESLOT60[resultFrame$DIRECTION == "GRi"], 
                     resultFrame$SYS[resultFrame$DIRECTION == "GRi"]), 
               file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables/STA_", staNames[i], "_GRi_",
                             lBTS, "-", fBTS,".csv"), 
               row.names = T)
    
    write.csv2(table(resultFrame$TIMESLOT120[resultFrame$DIRECTION == "Ri"], 
                     resultFrame$SYS[resultFrame$DIRECTION == "Ri"]), 
               file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables120/STA_", staNames[i], "_Ri_",
                             fBTS, "-", lBTS,".csv"), 
               row.names = T)
    
    write.csv2(table(resultFrame$TIMESLOT120[resultFrame$DIRECTION == "GRi"], 
                     resultFrame$SYS[resultFrame$DIRECTION == "GRi"]), 
               file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables120/STA_", staNames[i], "_GRi_",
                             lBTS, "-", fBTS,".csv"), 
               row.names = T)
}

staFiles <- list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables120/", full.names = T, pattern = ".csv$")
staNames <- gsub(".csv", "", list.files(path = "./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/tables120/", full.names = F, pattern = ".csv$"))
...
for(i in 1:length(staFiles)){
    ...
    write.csv2(resultFrame, file = paste0("./bottomup/merge_a(v)_v9/optimizedTrains/REM_9/tagesgang/filledTable120/FILLTABLE120_", staNames[i],".csv"), row.names = F)
}
