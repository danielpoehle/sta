...
optimizeTrains <- function(fileNames, z, t10, a_frame, num_trains){
    ...
    write.lp(lprec, paste0("./bottomup/merge_a(v)_v9/mip_", num_trains,".lp"), type = "lp", use.names = c(TRUE, TRUE))
...
files <- list.files(path = "./result_detail_v9/all90/", full.names = T, pattern = ".csv$")
fileNames <- list.files(path = "./result_detail_v9/all90/", full.names = F, pattern = ".csv$")
...
calc <- list()
for(num_trains in c(seq(8,500,1), length(z))){
    print(num_trains)
...
for(j in 1:length(calc)){
    ...
    write.csv2(completeFrame, file = paste0("./bottomup/merge_a(v)_v9/AllSelections_OPTI_", calc[[j]]$num_trains,".csv"), row.names = F)
    write.csv2(calc[[j]]$opt_z, file = paste0("./bottomup/merge_a(v)_v9/ModelTrains_OPTI_", calc[[j]]$num_trains, ".csv"), row.names = F)
}
...
for(j in 1:(length(calc)-1)){
    ...
    x <- read.csv2(file = paste0("./bottomup/merge_a(v)_v9/AllSelections_OPTI_", calc[[j]]$num_trains,".csv"), stringsAsFactors = F)
...
ggsave("./bottomup/merge_a(v)_v9/T10sum.png", e)

f <- qplot(x = n_sel, y = sum_t10) + theme_bw()
ggsave("./bottomup/merge_a(v)_v9/T10sum-n_sel.png", f)
...
for(i in 1:length(num_sel)){
    ...
    selected_files <- c(selected_files, paste0("./bottomup/merge_a(v)_v9/AllSelections_OPTI_", calc[[j]]$num_trains,".csv"))
}

write.csv2(selected_files, file = paste0("./bottomup/merge_a(v)_v9/SelectedFiles_Opti_v01.csv"), row.names = F)
